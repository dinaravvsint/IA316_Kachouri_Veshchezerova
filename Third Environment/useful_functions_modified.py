...
def compute_price_norm (state_history):
...
def sample_quintuplets(pos_data, state_history, random_seed=0):
    ...
    features_pos_items =[]
    features_neg_items = []
    features_items = [state_history[:][:][0][i][5:-1] for i in range(30)]
    rng = np.random.RandomState(random_seed)
    user_ids = pos_data['user_id'].values
    pos_item_ids = pos_data['item_id'].values
...
def sample_quintuplets_price(pos_data, state_history, random_seed=0):
    ...
    feat_items = [state_history[:][:][0][i][5:-1] for i in range(30)]
    price_mean = np.asarray(prices).mean()
    price_std = np.asarray(prices).std()
...
def build_models(n_users, n_items, user_dim=32, item_dim=30, n_hidden=1, hidden_size=64, dropout=0, l2_reg=0, 
    ...
    triplet_loss =  Lambda(loss, output_shape=(1,),
    ...
    deep_match_model = Model(input=[user_input, negative_item_input],
                             output=negative_similarity)
...
def build_models_covariates(n_users, n_items, user_dim=32, item_dim=30, n_hidden=1, hidden_size=64, dropout=0, l2_reg=0, 
    ...
    # adding features 
    features_pos_item_input = Input(shape=[1,2],name='feature_pos_item_input')
    features_neg_item_input = Input(shape=[1,2],name='feature_neg_item_input')
    ...
    interaction_layers = make_interaction_mlp(user_dim + item_dim+4, n_hidden=n_hidden, hidden_size=hidden_size,
                                                dropout=dropout, l2_reg=l2_reg)
